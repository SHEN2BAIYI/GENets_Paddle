...
def genet_large(pretrained=False, num_classes=1000):
...
def genet_normal(pretrained=False, num_classes=1000):
    this_file_dir = os.path.dirname(__file__)
    plainnet_txt = os.path.join(this_file_dir, 'GENet_normal.txt')
    if not os.path.isfile(plainnet_txt):
        raise RuntimeError(
            'Cannnot find {}!'.format(plainnet_txt))
    with open(plainnet_txt, 'r') as fid:
        plainnet_struct = fid.readlines()[0].strip()
...
def genet_light(pretrained=False, num_classes=1000):
    this_file_dir = os.path.dirname(__file__)
    plainnet_txt = os.path.join(this_file_dir, 'GENet_light.txt')
    if not os.path.isfile(plainnet_txt):
        raise RuntimeError(
            'Cannnot find {}!'.format(plainnet_txt))
    with open(plainnet_txt, 'r') as fid:
