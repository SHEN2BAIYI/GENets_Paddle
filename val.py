...
from utils.pytorchDataset import *
from torch.utils.data import DataLoader
from model.genet_pytorch import *

imagenet_data_dir = os.path.expanduser('./data/ILSVRC2012mini')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--params_dir', type=str, default='./weight/pytorch/GENet_large.pth',
    ...
    if opt.arch == 'GENet_large':
        ...
        model = genet_large(pretrained=opt.params_dir)
        # model = GENet.genet_large(pretrained=True, root=opt.params_dir)
    if opt.arch == 'GENet_normal':
        input_image_size = 192
        model = genet_normal(pretrained=opt.params_dir)
    if opt.arch == 'GENet_light':
        input_image_size = 192
        model = genet_light(pretrained=opt.params_dir)
    ...
    val_dataset = PytorchDataset('./data/ILSVRC2012mini', transformer)
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            if i % 100 == 0:
                print('mini_batch {}, top-1 acc={:4g}%, top-5 acc={:4g}%, number of evaluated images={}'.format(i,
                                                                                                                acc1[0],
                                                                                                                acc5[0],
                                                                                                                n))
            pass
    ...
    print(
        '*** arch={}, validation top-1 acc={}%, top-5 acc={}%, number of evaluated images={}, speed={:4g} img/s'.format(
